...
turn_num_to_percent_change<-function(input_dataset,numeric_variable, accuracy=0.1){
     if (class(input_dataset)[1]=="numeric"){
      ...
      output_dataset<-ifelse(stringr::str_detect(output_dataset,"-"), output_dataset,
                                       ifelse(is.na(output_dataset) , "NA",paste0("+",output_dataset)))
      output_dataset<-ifelse(output_dataset=="+Inf" , "+Inf",
      ...
      output_dataset <- input_dataset %>%
        ...
        dplyr::mutate(!!numeric_variable := ifelse(stringr::str_detect(!!as.name(numeric_variable),"-") , !!as.name(numeric_variable),
                                                           ifelse(is.na(!!as.name(numeric_variable)) , "NA", paste0("+",!!as.name(numeric_variable))))) %>%
        dplyr::mutate(!!numeric_variable := ifelse(!!as.name(numeric_variable)=="+Inf" , "+Inf",
